...
sudo pacman -S --noconfirm base base-devel linux linux-firmware reflector sudo man-db man-pages texinfo networkmanager curl git chromium grub efibootmgr amd-ucode dkms linux-headers xorg xorg-server gnome gnome-tweaks rustup noto-fonts noto-fonts-cjk noto-fonts-emoji noto-fonts-extra cups bluez bluez-utils filezilla thunderbird

yay -S --noconfirm gnome-shell-extension-pop-shell yay visual-studio-code-bin minecraft-launcher slack-desktop otf-hasklig neovim-nightly-bin
